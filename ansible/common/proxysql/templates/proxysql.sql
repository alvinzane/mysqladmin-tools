...
INSERT INTO mysql_galera_hostgroups(writer_hostgroup, backup_writer_hostgroup, reader_hostgroup, offline_hostgroup, active, max_transactions_behind)
 VALUES ({{mysql_port}}10,{{mysql_port}}11,{{mysql_port}}20,{{mysql_port}}90,1,300);
{% endif %}
...
{% if rep_type == 'rep' %}
 ...
 VALUES ({{mysql_port}}10,{{mysql_port}}20,'read_only');
{% endif %}
...
-- 读写分离
INSERT INTO mysql_query_rules (username,active,match_digest,destination_hostgroup,apply)
VALUES
('{{mysql_app_user}}',1,'^SELECT.*FOR UPDATE$',{{mysql_port}}10,1),
('{{mysql_app_user}}',1,'^SELECT',{{mysql_port}}20,1);
